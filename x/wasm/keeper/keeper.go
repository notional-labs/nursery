...
func (k Keeper) importAutoIncrementID(ctx sdk.Context, keyID []byte, val uint64) error {
	store := ctx.KVStore(k.storeKey)
	if store.Has(keyID) {
		return sdkerrors.Wrapf(types.ErrDuplicate, "autoincrement id: %s", string(keyID))
	}
